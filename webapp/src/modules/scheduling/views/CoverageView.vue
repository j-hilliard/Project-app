<template>
    <ModulePageShell>
        <ModulePageHeader title="Craft Coverage" subtitle="Demand vs assigned headcount by craft. Single-click to select — double-click to drill down.">
        ...
        <ModuleFilterBar>
            <div class="demand-toggle-group">
                ...
                <div class="demand-toggle">
                    <button v-for="opt in demandStateOptions" :key="opt.value"
                        class="toggle-btn" :class="{ active: demandState === opt.value }"
                        @click="demandState = opt.value">
                        {{ opt.label }}
                    </button>
                </div>
            </div>
            <Dropdown v-model="regionFilter" :options="regionOptions" optionLabel="label" optionValue="value"
                placeholder="All Regions" showClear class="filter-dropdown" />
            <Dropdown v-model="branchFilter" :options="branchOptions" optionLabel="label" optionValue="value"
                placeholder="All Branches" showClear class="filter-dropdown" />
        ...
        <!-- Craft Detail Drawer -->
        <Sidebar v-model:visible="drawerVisible" position="right"
            :style="{ width: 'clamp(380px, 40vw, 520px)' }" class="craft-detail-sidebar">
            <template #header>
            ...
            <CraftDetailDrawer v-if="drawerCraft" :craft="drawerCraft" :suggestions="suggestionsForDrawer" />
        </Sidebar>
    </ModulePageShell>
</template>
...
import { ModulePageShell, ModulePageHeader, ModuleFilterBar } from '@/ui';
...
const { getCoverage, getSuggestedMatches } = useCoverageService();
...
const displayRows = computed(() => {
    ...
    if (demandState.value !== 'all') {
        ...
        if (stateAware.length) rows = rows.filter(r => r.demandState === demandState.value);
    }
    if (regionFilter.value) {
        const regionCrafts = new Set(allMatches.value.filter((m: any) => m.region === regionFilter.value).map((m: any) => m.craftCode).filter(Boolean));
        if (regionCrafts.size) rows = rows.filter(r => regionCrafts.has(r.craftCode));
    }
    if (branchFilter.value) {
        const branchCrafts = new Set(allMatches.value.filter((m: any) => m.branch === branchFilter.value).map((m: any) => m.craftCode).filter(Boolean));
...
const openPositions = computed(() => normalizedRows.value.reduce((s, r) => s + Math.max(0, r.gap), 0));
const totalAssigned = computed(() => normalizedRows.value.reduce((s, r) => s + (r.assigned ?? 0), 0));
const totalDemand = computed(() => normalizedRows.value.reduce((s, r) => s + (r.demand ?? 0), 0));
...
.kpi-item { display: flex; flex-direction: column; align-items: center; flex: 1; padding: 0 1rem; }
...
.kpi-label { font-size: 0.68rem; color: var(--text-color-secondary); text-transform: uppercase; letter-spacing: 0.06em; margin-top: 0.25rem; text-align: center; }
.kpi-warn { color: var(--red-500, #ef4444); }
.kpi-mid  { color: var(--yellow-500, #eab308); }
.kpi-ok   { color: var(--green-500, #22c55e); }

.demand-toggle-group { display: flex; align-items: center; gap: 0.5rem; flex-shrink: 0; }
.filter-label-sm { font-size: 0.72rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; color: var(--text-color-secondary); }
.demand-toggle { display: flex; background: var(--surface-ground); border-radius: 6px; padding: 2px; gap: 1px; }
.toggle-btn { background: transparent; border: none; color: var(--text-color-secondary); font-size: 0.75rem; font-weight: 600; padding: 0.25rem 0.7rem; border-radius: 5px; cursor: pointer; transition: all 0.12s ease; }
...
.filter-hint { margin-left: auto; font-size: 0.75rem; color: var(--text-color-secondary); display: flex; align-items: center; gap: 0.35rem; }

.coverage-dashboard { display: flex; flex-direction: column; gap: 0.5rem; }
.dashboard-col-labels { display: flex; align-items: center; gap: 1rem; padding: 0 1.25rem; font-size: 0.65rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: var(--text-color-secondary); }
.col-lbl-craft { min-width: 220px; flex-shrink: 0; }
.col-lbl-bar { flex: 1; }
.col-lbl-stats { min-width: 300px; text-align: right; flex-shrink: 0; }

.skeleton-row { height: 58px; background: var(--surface-card); border: 1px solid var(--surface-border); border-radius: 10px; animation: pulse 1.5s ease-in-out infinite; }
@keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.4; } }

.empty-state { text-align: center; padding: 3rem 1rem; color: var(--text-color-secondary); font-size: 0.88rem; }
...
.sidebar-header { display: flex; align-items: center; gap: 0.5rem; font-weight: 700; font-size: 0.95rem; color: var(--text-color); }
:deep(.craft-detail-sidebar .p-sidebar-header) { padding: 1rem 1.25rem 0.75rem; border-bottom: 1px solid var(--surface-border); }
:deep(.craft-detail-sidebar .p-sidebar-content) { padding: 1rem 1.25rem; overflow-y: auto; }
</style>
