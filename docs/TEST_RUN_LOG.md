...
| 2026-04-29 | Cleanup continuation QA guardrails | QA docs update only | Record Joseph's rejection of "Batches 1-5 complete" and add Batches 6-10 acceptance gates | PASS / DOCS ONLY | Added `ARCH-009` through `ARCH-014` to `LIVE_QA_TODO.md`; added `QA-ARCH-012` through `QA-ARCH-015`, `QA-UI-015`, and `QA-UI-016` regression expectations. No product code changed. |
| 2026-04-29 | Project-app QA enforcement charter | QA docs update only | Add Joseph's QA/regression/architecture enforcement charter to Codex operating rules | PASS / DOCS ONLY | Added charter to `QA_REGRESSION_CHECKLIST.md`, added `ARCH-015` to `LIVE_QA_TODO.md`, and added regression checks for interaction states, Scheduling personnel-only boundary, Planning Gantt/timeline ownership, field-safe WorkPackage surfaces, and Estimate -> WorkOrder -> FCO -> Actuals traceability. No product code changed. |
| 2026-04-29 | Project-app QA enforcement charter | QA docs update only | Mirror Joseph's charter into the top-level Live QA rules | PASS / DOCS ONLY | Added a `Project-app QA enforcement charter` rule to `LIVE_QA_TODO.md`: read live TODO/checklist first, inspect each Claude batch, run architecture/runtime/route/interaction checks, verify screenshots, log findings, and require Joseph approval plus Codex retest before closure. No product code changed. |
---
